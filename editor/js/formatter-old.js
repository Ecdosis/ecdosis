...
function Link(text,next,prev)
{
    ...
    /** 
     * Insert NL after first HTML end-tag
     * @param str the HTML string to modify
     * @return the modified str
     */
    this.insertNL = function(str) {
        var newStr = "";
        var state = 0;
    ...
    this.toHtml = function() {
        ...
        while ( temp != null )
        {
            var trailingNL = false;
            html += temp.preHtml;
            if ( temp.text.length>0 && temp.text[temp.text.length-1] == '\n')
            {
                trailingNL = true;
                temp.text = temp.text.slice(0,temp.text.length-1);
            }
            html += temp.text;
            if ( trailingNL )
                html += this.insertNL(temp.postHtml);
            else
                html += temp.postHtml;
            temp = temp.next;
        }
...
function Formatter( dialect ) 
{
    ...
    */
                ...
                if ( pfmt.leftTag != undefined && pfmt.rightTag != undefined )
                {
                    ...
                    if ( lpos != -1 )
                    {
                        ...
                        while ( last != para && last != null )
                        {
                            ...
                            if ( rpos != -1 )
                            {
                                var tag = this.cssMap[pfmt.prop];
                                if ( tag == undefined )
                                {
                                    tag = "p";
                                    console.log("couldn't find tag for "+pfmt.prop);
                                }
                                line.setText(line.text.substr(lpos+ltag.length));
                                line.prependHtml('<'+tag+' class="'+pfmt.prop+'"title="'+pfmt.prop+'">');
                                ...
                                // suppress adding of default para tags
    ...
    this.isHeading = function( line, c )
    {
        ...
        {
            if ( trimmed.charAt(j) !=c )
                break;  
        }
        return j == trimmed.length;
    };
    /**
    ...
    this.processLineformat = function( lf, link ) {
        ...
        if ( lf.rightTag.length > 0 )
        {
            ...
            rhSlice = true; // removed trailing NL
        }
        link.setText(line.slice(lf.leftTag.length,end));
        if ( hasLF && rhSlice && lf.prop != "page" )
        {
            link.text += '\n';       
        } 
    };
